...
svm.2=function(X,y,C, K){ ## 関数名をsvmとした.2
...
# 関数の定義
plot.kernel=function(K, lty){ ## 引数で，線の種類を指定するlty
 ...
 ## は，f(x,y)での方向の高さを求める。これから，輪郭を求めることができる。z
...
boxplot(u[,1],u[,2],u[,3], ylab="k(x,y)との差異",names=paste0("m=",m_seq),
        col=c("red","blue","green"), main="RFFによるカーネルの近似")
...
n=300; x=rnorm(n)/2; y=3-2*x^2+3*x^3+2*rnorm(n)       ## データ生成
